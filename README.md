...
|------------------------|--------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `DB_HOST`              | `postgres`                     | Адрес расположения БД. Используется бэкендом для подключения к базе данных                                                                                                                                                                                                                                                                               |
...
| `API_URL`              | `http://localhost:3000/api/v1` | Внешний URL бэкенда. **ВАЖНО:** при деплое в интернет URL **обязательно** должен быть доступен извне. Данный URL используется фронтендом для отправки запросов бэкенду                                                                                                                                                                                   |
...
| `FRONTEND_PORT`        | `80`                           | Внешний порт работы фронтенда. Используется докером для прокидывания портов                                                                                                                                                                                                                                                                              |


При первом запуске произойдет инициализация БД, создание суперпользователя и заполнение БД пустыми таблицами.
Далее необходимо создать нового пользователя в PostgreSQL в роли ученика, учителя или админа. Для этого необходимо выполнить следующие команды:
```bash
sudo docker exec -it hibernia_db psql -U changeme -d hibernia
# Фамилия, Имя, Отчество, Возраст, Пол (0 - Мужской, 1 - Женский), Стаж, Логин, Пароль
CALL add_teacher('Иванов'::text, 'Иван'::text, 'Иванович'::text, 30, 0::bit, 10::numeric, 'ivanov'::text, '12345'::text);
```
После этого можно перейти по адресу `http://localhost` и войти в аккаунт от имени учителя. Создание учеников происходит во вкладке "Список студентов" \
Документация находится по адресу `http://localhost:8080`
